...
@section('content')
    <div
        class="flex h-[80vh] max-w-6xl mx-auto bg-black/60 backdrop-blur-xl border border-gray-800/60 rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div class="flex-1 overflow-y-auto">
                @forelse($members as $m)
                    <a href="{{ route('trainer.communication.chat.index', ['user' => $m->id]) }}"
                        ...
                        <div class="relative">
                            <div
                                class="w-10 h-10 rounded-full bg-emerald-400/20 flex items-center justify-center text-emerald-400 font-bold">
        ...
        <div class="flex-1 flex flex-col bg-gray-950/70 backdrop-blur-xl">
            @if(isset($user))
                    ...
                    @forelse($chats as $chat)
                        ...
                        @if($chat->sender_type === 'trainer')
                            <div class="flex justify-end relative chat-message" id="chat-{{ $chat->id }}">
                                <div class="text-right max-w-[75%]">
                                    <div
                                        class="bg-gradient-to-r from-emerald-500 to-cyan-500 text-black font-medium px-4 py-2 rounded-2xl rounded-br-none inline-block shadow-lg shadow-emerald-500/30">
                                        {{ $chat->message }}
                                    </div>
                        ...
                        @elseif($chat->sender_type === 'user')
                            <div class="flex items-start gap-2 chat-message">
                                <div
                                    class="w-8 h-8 bg-gray-800/70 rounded-full flex items-center justify-center text-gray-300 text-sm font-bold">
                                    U
                                </div>
                                <div>
                                    <div
                                        class="bg-gray-900/80 border border-gray-700 text-white px-4 py-2 rounded-2xl rounded-bl-none inline-block max-w-[75%] shadow-md">
                                        {{ $chat->message }}
                                    </div>
                ...
                {{-- INPUT BAR --}}
                <form id="chat-form"
                    class="flex items-center gap-3 p-4 bg-gray-900/60 border-t border-gray-800/60 backdrop-blur-md">
                    @csrf
            ...
            @else
                <div class="flex-1 flex flex-col items-center justify-center bg-gray-950/70 text-gray-400">
                    <div class="text-center p-6">
                        ...
                        <p class="text-sm text-gray-500">Pesan akan tampil di sini setelah kamu memilih salah satu member di
                            sebelah kiri.</p>
                    </div>
                </div>
            @endif
