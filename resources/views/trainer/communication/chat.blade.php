        ...
        @if(isset($user))
            (function () {
                ...
                document.addEventListener('click', async (e) => {
                    const el = e.target.closest('.delete-chat');
                    if (!el) return;
                    ...
                    // Animation
                    chatEl.style.transition = 'all .3s ease';
                    chatEl.style.opacity = '0';
                    chatEl.style.transform = 'translateY(20px) scale(0.9)';
                ...
                setInterval(async () => {
                    try {
                        const response = await axios.get(`{{ route('trainer.communication.chat.index', ['user' => $user->id]) }}?date={{ $dateFilter ?? '' }}`);
                        if (response.data) {
                            // Here you would update the chat messages
