...
<h1 class="font-serif text-3xl font-bold text-white mb-6">
    🏋️ Program <span class="text-emerald-400">Latihan Member</span>
</h1>
...
<div class="bg-black/70 backdrop-blur-lg border border-gray-700/50 overflow-hidden shadow-sm sm:rounded-lg">
            ...
            <table class="min-w-full divide-y divide-gray-700">
                <thead class="bg-gray-900/50">
                    <tr class="text-left font-semibold text-gray-400">
                        <th class="px-6 py-3 text-xs font-medium uppercase tracking-wider">Nama Member</th>
                        <th class="px-6 py-3 text-xs font-medium uppercase tracking-wider">Status Program</th>
                        <th class="px-6 py-3 text-xs font-medium uppercase tracking-wider">Durasi (Minggu)</th>
                        <th class="px-6 py-3 text-center text-xs font-medium uppercase tracking-wider">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-800">
                    @foreach($members as $member)
                        @php $plan = $member->workoutPlans->last() ?? null; @endphp
                        <tr class="hover:bg-gray-800/50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <a href="{{ route('trainer.programs.edit', ['memberId' => $member->id]) }}"
                                   class="text-emerald-400 hover:text-emerald-300 font-medium text-sm">
                                    ✏️ Edit Program
                                </a>
                            </td>
                        </tr>
    ...
    @else
        <p class="text-gray-400 italic text-center p-6">
            Belum ada member yang terdaftar di bawah bimbingan Anda.
        </p>
    @endif
