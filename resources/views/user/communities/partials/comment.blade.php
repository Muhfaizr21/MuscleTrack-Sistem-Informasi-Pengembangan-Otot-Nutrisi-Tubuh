<div class="comment-card" id="comment-{{ $comment->id }}">
    <div class="flex items-start justify-between mb-2">
        ...
        @if(isset($userRole) && ($comment->user_id === auth()->id() || $userRole === 'admin' || $userRole === 'moderator'))
            <div class="flex items-center gap-2">
                ...
                @endif
                <form action="{{ route('user.communities.comments.destroy', $comment->id) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
    ...
    <div id="edit-comment-form-{{ $comment->id }}" class="hidden mt-3">
        <form action="{{ route('user.communities.comments.update', $comment->id) }}" method="POST">
            @csrf
            @method('PUT')
    ...
    <div id="reply-form-{{ $comment->id }}" class="hidden mt-3">
        <form action="{{ route('user.communities.comments.store', $comment->post_id) }}" method="POST">
            @csrf
...
@if($comment->replies && $comment->replies->count() > 0)
    <div class="ml-6 mt-3 space-y-3 border-l-2 border-emerald-500/20 pl-4">
        @foreach($comment->replies as $reply)
            @include('user.communities.partials.comment', ['comment' => $reply, 'userRole' => $userRole ?? null])
        @endforeach
    </div>
@endif