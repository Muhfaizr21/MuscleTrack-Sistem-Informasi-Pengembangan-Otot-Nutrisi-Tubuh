                            ...
                            <div class="flex items-center justify-end gap-3">
                                ...
                                @if(isset($article->slug))
                                    <a href="{{ route('public.articles.show', $article->slug) }}"
                                       target="_blank"
                                       class="text-blue-400 hover:text-blue-300 transition-colors duration-300 flex items-center gap-1 p-2 rounded-lg hover:bg-blue-500/10"
                                       title="Lihat Artikel">
                                ...
                                <a href="{{ route('admin.articles.edit', $article) }}"
                                   class="text-green-400 hover:text-green-300 transition-colors duration-300 flex items-center gap-1 p-2 rounded-lg hover:bg-green-500/10"
                                   title="Edit Artikel">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                ...
                                <!-- Delete Article -->
                                <form action="{{ route('admin.articles.destroy', $article) }}" method="POST" class="inline m-0">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                            onclick="return confirm('Yakin ingin menghapus artikel \"{{ $article->title }}\"?')"
                                            class="text-red-400 hover:text-red-300 transition-colors duration-300 flex items-center gap-1 p-2 rounded-lg hover:bg-red-500/10"
                                            title="Hapus Artikel">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
