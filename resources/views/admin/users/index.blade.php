<x-layouts.admin>
    ...
    <div class="bg-slate-800/40 backdrop-blur-lg border border-slate-700/30 rounded-2xl shadow-2xl shadow-black/30 overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-700/50">
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                ...
                <a href="{{ route('admin.users.create') }}"
                   class="px-6 py-3 rounded-xl bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold shadow-lg shadow-green-500/20 hover:shadow-green-500/40 transition-all duration-300 transform hover:-translate-y-0.5 flex items-center gap-2">
            ...
            <form method="GET" action="{{ route('admin.users.index') }}" class="space-y-4">
                ...
                <div class="flex flex-col sm:flex-row gap-3 justify-between items-center">
                    <div class="text-sm text-slate-400">
                        Menampilkan <span class="font-semibold text-green-400">{{ $users->firstItem() }}</span> - <span class="font-semibold text-green-400">{{ $users->lastItem() }}</span> dari total <span class="font-semibold text-green-400">{{ $users->total() }}</span> user
                    ...
                    <div class="flex gap-3">
                        <button type="submit"
                                class="px-6 py-2 rounded-xl bg-green-600 text-white font-semibold shadow-md hover:bg-green-700 transition-colors duration-300 flex items-center gap-2">
                        ...
                        <a href="{{ route('admin.users.index') }}"
                           class="px-6 py-2 rounded-xl border border-slate-600/50 text-slate-300 hover:text-white hover:bg-slate-700/50 transition-colors duration-300 flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <table class="min-w-full divide-y divide-slate-700/50">
                ...
                <tbody class="divide-y divide-slate-700/30">
                    ...
                    <tr class="hover:bg-slate-700/20 transition-colors duration-300">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @php
                                $statusData = match($user->verification_status) {
                                    'approved' => ['text' => 'Aktif', 'class' => 'bg-green-500 text-green-100', 'dot' => 'bg-green-400'],
                                    'pending' => ['text' => 'Pending', 'class' => 'bg-yellow-500 text-yellow-100', 'dot' => 'bg-yellow-400'],
                                    default => ['text' => 'Ditolak', 'class' => 'bg-red-500 text-red-100', 'dot' => 'bg-red-400'],
                                };
                            @endphp
                             <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-slate-700/50 border border-slate-600">
                                <span class="w-2 h-2 mr-2 rounded-full {{ $statusData['dot'] }}"></span>
                                <span class="text-slate-300">{{ $statusData['text'] }}</span>
                            </span>
                        </td>
                        ...
                        <td class="px-6 py-4 text-sm text-slate-300">
                            <div class="space-y-1 text-xs">
                                <p class="flex items-center gap-1">
                                    <span class="text-slate-500 w-16">Usia/JK:</span>
                                    <span class="font-medium text-white">{{ $user->age ?? '-' }} Thn, {{ $user->gender ? ucfirst($user->gender) : '-' }}</span>
                                </p>
                                <p class="flex items-center gap-1">
                                    <span class="text-slate-500 w-16">BB/TB:</span>
                                    <span class="font-medium text-white">{{ $user->weight ?? '-' }} kg / {{ $user->height ?? '-' }} cm</span>
                                </p>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end gap-3">
                                <a href="{{ route('admin.users.show', $user) }}"
                                   title="Lihat Detail"
                                   class="text-blue-400 hover:text-blue-300 transition-colors duration-300 p-1 rounded-full hover:bg-slate-700/50">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>

                                <a href="{{ route('admin.users.edit', $user) }}"
                                   title="Edit Data"
                                   class="text-green-400 hover:text-green-300 transition-colors duration-300 p-1 rounded-full hover:bg-slate-700/50">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="inline">
                                    ...
                                    <button type="submit"
                                            title="Hapus User"
                                            onclick="return confirm('Anda yakin ingin menghapus user {{ $user->name }}? Aksi ini tidak dapat dibatalkan.')"
                                            class="text-red-400 hover:text-red-300 transition-colors duration-300 p-1 rounded-full hover:bg-slate-700/50">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-6 border-t border-slate-700/50">
            {{ $users->links('pagination::tailwind') }} </div>
    </div>
