<x-layouts.admin>
    <x-slot name="title">
        Community <span class="bg-gradient-to-r from-purple-400 to-indigo-500 bg-clip-text text-transparent">Statistics</span>
    </x-slot>
    ...
    <div class="space-y-6">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-purple-400/80 text-lg mt-2">Analisis mendalam komunitas MuscleXpert</p>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-slate-800/40 backdrop-blur-lg border border-orange-500/20 rounded-2xl p-6 shadow-lg shadow-orange-500/10">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-gray-400 text-sm font-medium mb-2">Communities Baru</p>
                        <p class="text-white text-3xl font-bold">{{ $recentCommunities }}</p>
                        <p class="text-orange-400 text-xs mt-2">7 hari terakhir</p>
                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-slate-800/40 backdrop-blur-lg border border-purple-500/20 rounded-2xl p-6 shadow-lg shadow-purple-500/10">
                <h3 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
                    <span class="text-purple-400">🌐 Tipe Communities</span>
                </h3>
                <div class="space-y-4">
                    <div class="flex items-center justify-between p-3 rounded-xl bg-slate-800/50">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-white font-medium text-sm">Public Communities</p>
                                <p class="text-gray-400 text-xs">Terbuka untuk semua</p>
                            </div>
                        </div>
                        <span class="text-green-400 text-lg font-bold">{{ $publicCommunities }}</span>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-3 rounded-xl bg-slate-800/50">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-white font-medium text-sm">Private Communities</p>
                                <p class="text-gray-400 text-xs">Hanya untuk anggota</p>
                            </div>
                        </div>
                        <span class="text-blue-400 text-lg font-bold">{{ $privateCommunities }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-slate-800/40 backdrop-blur-lg border border-emerald-500/20 rounded-2xl p-6 shadow-lg shadow-emerald-500/10">
                <h3 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
                    <span class="text-emerald-400">📈 Statistik Pertumbuhan</span>
                </h3>
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="text-center p-4 rounded-xl bg-emerald-500/10">
                            <p class="text-emerald-400 text-2xl font-bold">{{ $recentCommunities }}</p>
                            <p class="text-gray-400 text-xs mt-1">Communities Baru</p>
                        </div>
                        <div class="text-center p-4 rounded-xl bg-blue-500/10">
                            <p class="text-blue-400 text-2xl font-bold">{{ $totalMembers }}</p>
                            <p class="text-gray-400 text-xs mt-1">Total Anggota</p>
                        </div>
                    </div>
                    <div class="p-3 rounded-xl bg-slate-800/50 text-center">
                        <p class="text-gray-400 text-sm">
                            <span class="text-emerald-400 font-bold">{{ number_format($totalMembers / max($totalCommunities, 1), 1) }}</span>
        ...
        <div class="bg-slate-800/40 backdrop-blur-lg border border-purple-500/20 rounded-2xl p-6 shadow-lg shadow-purple-500/10">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-bold text-white flex items-center gap-2">
                    <span class="text-purple-400">🏆 Top 10 Communities</span>
                </h3>
                <span class="text-gray-400 text-sm">Berdasarkan jumlah anggota</span>
            </div>

            <div class="space-y-3">
                @foreach($topCommunities as $index => $community)
                <div class="flex items-center justify-between p-4 rounded-xl bg-slate-800/50 hover:bg-slate-700/50 transition-all duration-300 group">
                    <div class="flex items-center gap-4 flex-1">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 flex items-center justify-center bg-gradient-to-br from-purple-500 to-indigo-600 rounded-lg text-white font-bold text-sm">
                                {{ $index + 1 }}
                            </div>
                            @if($community->image)
                                <img src="{{ Storage::url($community->image) }}" alt="{{ $community->name }}" class="w-12 h-12 rounded-xl object-cover">
                            @else
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-white font-medium text-sm truncate">{{ $community->name }}</p>
                            <p class="text-gray-400 text-xs mt-1">
                                {{ $community->posts_count }} posts •
                                Dibuat {{ $community->created_at->diffForHumans() }}
                            </p>
                        </div>
                    </div>
                    <div class="flex items-center gap-4">
                        <div class="text-right">
                            <p class="text-white font-bold text-lg">{{ $community->members_count }}</p>
                            <p class="text-purple-400 text-xs">members</p>
                        </div>
        ...
        <div class="bg-slate-800/40 backdrop-blur-lg border border-cyan-500/20 rounded-2xl p-6 shadow-lg shadow-cyan-500/10">
            <h3 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
                <span class="text-cyan-400">🧭 Navigasi Cepat</span>
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <a href="{{ route('admin.communities.dashboard') }}" class="p-4 rounded-xl bg-cyan-500/10 border border-cyan-500/20 hover:bg-cyan-500/20 transition-all duration-300 group">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-cyan-400 text-xs">Overview komunitas</p>
                        </div>
                    </div>
                </a>

                <a href="{{ route('admin.communities.index') }}" class="p-4 rounded-xl bg-purple-500/10 border border-purple-500/20 hover:bg-purple-500/20 transition-all duration-300 group">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-purple-400 text-xs">Lihat semua komunitas</p>
                        </div>
                    </div>
                </a>
